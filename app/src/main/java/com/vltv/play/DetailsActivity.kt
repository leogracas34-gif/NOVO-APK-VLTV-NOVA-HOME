...
class DetailsActivity : AppCompatActivity() {
    ...
    private fun carregarConteudo() {
        ...
        if (isSeries) {
            ...
            if (serverYoutubeTrailer != null) {
                val extrasList = listOf(EpisodeData(0, 0, 1, "Trailer Oficial",
                    "https://img.youtube.com/vi/$serverYoutubeTrailer/0.jpg", serverYoutubeTrailer))
                episodesAdapter.submitList(extrasList)
                recyclerEpisodes.visibility =
                    if (tabLayoutDetails.selectedTabPosition == 1) View.VISIBLE else View.GONE
            }
        }
        verificarResume()
        ...
        // Verifica se tem mais de 30 segundos e se a duração total é válida
        if (pos >= 30_000L && totalDur > 0) {
            btnPlay.text = "▶  CONTINUAR"
            btnRestartAction?.visibility = View.VISIBLE
            layoutProgress?.visibility = View.VISIBLE

            // CÁLCULO CRONOMETRADO:
            // Multiplicamos por 100 antes de dividir para manter precisão em Long
    ...
    private fun iniciarMonitoramentoUI() {
        ...
        uiMonitorJob = CoroutineScope(Dispatchers.Main).launch {
            ...
            while (isActive) {
                val download = withContext(Dispatchers.IO) { db.getDownloadByStreamId(streamId, tipo) }
                if (download != null) {
    ...
    private fun sincronizarDadosTMDB() {
        ...
        listOf("FHD", "HD", "4K", "H265", "LEG", "DUBLADO")
            .forEach { clean = clean.replace(it, "", ignoreCase = true) }
                ...
                try {
                    ...
                    if (logos != null && logos.length() > 0) {
                        ...
                        for (i in 0 until logos.length()) {
                            ...
                            val reg  = lg.optString("iso_3166_1", "").trim().uppercase()
                            if (lang == "pt" && reg == "BR") { finalPath = lg.optString("file_path"); if (!finalPath.isNullOrEmpty()) break }
                        }
                        if (finalPath.isNullOrEmpty()) {
                            for (i in 0 until logos.length()) {
                                val lg   = logos.getJSONObject(i)
                                val lang = lg.optString("iso_639_1", "").trim().lowercase()
                                if (lang == "pt") { finalPath = lg.optString("file_path"); if (!finalPath.isNullOrEmpty()) break }
                            }
                        }
                        if (finalPath.isNullOrEmpty()) { runOnUiThread { tvTitle.visibility = View.VISIBLE; imgTitleLogo.visibility = View.GONE }; return }
                    ...
                    } else { runOnUiThread { tvTitle.visibility = View.VISIBLE; imgTitleLogo.visibility = View.GONE } }
                } catch (e: Exception) { runOnUiThread { tvTitle.visibility = View.VISIBLE; imgTitleLogo.visibility = View.GONE } }
            }
            override fun onFailure(call: Call, e: IOException) { runOnUiThread { tvTitle.visibility = View.VISIBLE; imgTitleLogo.visibility = View.GONE } }
        })
    }
        ...
        client.newCall(Request.Builder().url(detailsUrl).build()).enqueue(object : Callback {
            override fun onResponse(call: Call, response: Response) {
                ...
                try {
                    ...
                    if (castArray != null) { val limit = minOf(castArray.length(), 10); for (i in 0 until limit) castNames.add(castArray.getJSONObject(i).getString("name")) }
                    ...
                    var director   = "Desconhecido"
                    if (crew != null) for (i in 0 until crew.length()) { if (crew.getJSONObject(i).optString("job") == "Director") { director = crew.getJSONObject(i).getString("name"); break } }
                    var similarArr = d.optJSONObject("recommendations")?.optJSONArray("results")
                    if (similarArr == null || similarArr.length() == 0) similarArr = d.optJSONObject("similar")?.optJSONArray("results")
                    val suggestions = mutableListOf<JSONObject>()
                    if (similarArr != null) for (i in 0 until similarArr.length()) suggestions.add(similarArr.getJSONObject(i))
                    ...
                    if (serverYoutubeTrailer == null) {
                        ...
                        if (videosArr != null) for (i in 0 until videosArr.length()) {
                            ...
                            if (v.optString("site") == "YouTube") extrasList.add(EpisodeData(i, 0, i+1, v.getString("name"), "https://img.youtube.com/vi/${v.getString("key")}/0.jpg", v.getString("key")))
                        }
                    }
                    runOnUiThread {
                        val g = genresList.joinToString(", ")
                        val e = castNames.joinToString("\n")
        ...
        episodesAdapter = EpisodesAdapter { episode ->
            if (episode.videoKey != null) {
                ...
                startActivity(Intent(this, PlayerActivity::class.java).apply {
                    putExtra("stream_id",    episode.streamId)
                    putExtra("stream_type",  "series")
                    putExtra("channel_name", "${name} - S${episode.season}:E${episode.episode}")
                    putExtra("icon",         episode.thumb)
                    putExtra("PROFILE_NAME", currentProfile)
                })
            }
        }
        recyclerEpisodes.apply {
            layoutManager = if (isTelevisionDevice()) GridLayoutManager(this@DetailsActivity, 6)
                            else LinearLayoutManager(this@DetailsActivity, LinearLayoutManager.HORIZONTAL, false)
    ...
    private fun atualizarUI_download() {
        when (downloadState) {
            DownloadState.BAIXAR   -> { imgDownloadState.visibility = View.VISIBLE; pbDownloadCircular?.visibility = View.GONE; imgDownloadState.setImageResource(android.R.drawable.stat_sys_download); tvDownloadState.text = "BAIXAR" }
            DownloadState.BAIXANDO -> { imgDownloadState.visibility = View.GONE; pbDownloadCircular?.visibility = View.VISIBLE; tvDownloadState.text = "0%" }
            DownloadState.BAIXADO  -> { imgDownloadState.visibility = View.VISIBLE; pbDownloadCircular?.visibility = View.GONE; imgDownloadState.setImageResource(android.R.drawable.stat_sys_download_done); tvDownloadState.text = "BAIXADO" }
        }
    }
    ...
    private fun mostrarConfiguracoes() {
        ...
        AlertDialog.Builder(this).setTitle("Player").setItems(p) { _, i ->
            getSharedPreferences("vltv_prefs", Context.MODE_PRIVATE).edit().putString("player_preferido", p[i]).apply()
        }.show()
    }
    ...
    inner class SuggestionsAdapter(val items: List<JSONObject>) :
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
            ...
            val card = androidx.cardview.widget.CardView(parent.context).apply { layoutParams = LinearLayout.LayoutParams(130.toPx(), 190.toPx()); radius = 8f }
            val img  = ImageView(parent.context).apply { id = android.R.id.icon; scaleType = ImageView.ScaleType.CENTER_CROP }
            card.addView(img)
            val tv = TextView(parent.context).apply { id = android.R.id.text1; setTextColor(Color.WHITE); textSize = 10f; gravity = android.view.Gravity.CENTER; maxLines = 2 }
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            ...
            holder.itemView.setOnClickListener {
                startActivity(Intent(holder.itemView.context, DetailsActivity::class.java).apply {
                    putExtra("stream_id", item.optInt("id"))
                    putExtra("name",  item.optString("title"))
                    putExtra("icon",  "https://image.tmdb.org/t/p/w342$path")
                    putExtra("PROFILE_NAME", currentProfile)
                })
            }
        }
